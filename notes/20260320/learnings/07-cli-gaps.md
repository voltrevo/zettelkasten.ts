...
No CLI command exists to read back an atom's description or check what tests
are registered for it without raw curl. This caused agents to describe atoms
and then have no way to verify the description was stored.
...
```
Agents went through multiple post-reject-edit cycles to hit the size limit.
A client-side estimate (even an approximation) would short-circuit this.
...
A `zts-plus` wrapper at `~/.local/bin/zts-plus` was built in this container
